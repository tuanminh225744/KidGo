...
export default function DeviationView({ onNavigate }) {
  return (
    <div className="min-h-screen bg-white pb-24">
      <div className="flex items-center gap-3 p-4 border-b border-gray-50">
        <ChevronLeft className="w-6 h-6 text-gray-600 cursor-pointer" onClick={() => onNavigate('history')} />
      ...
      <div className="p-4">
        <div className="bg-orange-50/50 p-4 rounded-xl border border-orange-100 flex gap-4 mb-6 relative overflow-hidden">
          ...
          <div>
            <h3 className="font-bold text-orange-800 mb-2 leading-tight">Hệ thống phát hiện xe lệch 300m so với lộ trình đã đăng ký</h3>
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-xs text-orange-700">
                <Clock className="w-3 h-3" />
                <span>Thời gian: 14:35</span>
              </div>
              <div className="flex items-start gap-2 text-xs text-orange-700">
                <MapPin className="w-3 h-3 mt-0.5" />
                <span>Đường Nguyễn Hữu Cảnh, Quận 1</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-8">
          {[
          { id: 1, title: 'Tắc đường, phải đi đường vòng', icon: '🚦', active: true },
          ...
          <div
            key={reason.id}
            className={`p-4 rounded-2xl flex items-center justify-between border-2 transition-all ${reason.active ? 'bg-green-50/50 border-[#1D7C45]' : 'bg-gray-50/30 border-gray-100 hover:border-gray-200'}`}>
            
              <div className="flex items-center gap-4">
                <span className="text-xl">{reason.icon}</span>
                <span className={`font-bold text-sm ${reason.active ? 'text-gray-800' : 'text-gray-500'}`}>{reason.title}</span>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${reason.active ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
                {reason.active && <CheckCircle2 className="w-3 h-3 text-white" />}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 pt-4">
            <button
              className="w-full bg-[#1D7C45] text-white font-bold py-4 rounded-2xl shadow-lg shadow-green-100"
