...
export default function BottomNav() {
  ...
  const items = [
    ...
    { id: 'schedule', label: 'Lịch trình', icon: <Calendar className="w-6 h-6" /> },
    { id: 'history', label: 'Lịch sử', icon: <CalendarDays className="w-6 h-6" /> },
  ...
  return (
    <nav className="bg-white border-t border-gray-100 px-6 py-3 pb-8 flex justify-between items-center fixed bottom-0 left-0 right-0 max-w-md mx-auto z-20">
      {items.map((item) => {
        const isActive = currentView === item.id || (item.id === 'account' && currentView === 'profile');
        return (
          <button
            key={item.id}
