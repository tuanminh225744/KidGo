...
export default function PaymentScreen() {
  ...
  const { kidId, routeId, bookingDateTime, selectedDriverId, resetBooking } =
    useBookingStore();
  ...
  const handleConfirmSelection = async () => {
    ...
    try {
      ...
      const payment =
        payRes.data?.data?.payment || payRes.data?.payment || payRes.payment;
      if (!payment || !payment._id)
        throw new Error("Không lấy được ID thanh toán");
      ...
      <main className="flex-1 px-5 pt-6 pb-32 overflow-y-auto space-y-6">
          ...
          {isLoadingPrice ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-on-surface">
                  {(pricingInfo.pricePerTrip || 0).toLocaleString("vi-VN")} đ
                </span>
              </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-3">
            {isImmediate && (
              <label
                className={`flex items-center gap-4 p-4 rounded-3xl border-2 transition-all cursor-pointer ${paymentMethod === "cash" ? "border-primary bg-primary/5" : "border-outline-variant/30 bg-white hover:border-primary/30"}`}
              >
                <input
                  type="radio"
                  name="paymentMethod"
                ...
                />
                <div
                  className={`p-3 rounded-2xl ${paymentMethod === "cash" ? "bg-primary text-white" : "bg-surface-container text-on-surface-variant"}`}
                >
                  <Banknote size={24} />
                </div>
            ...
            <label
              className={`flex items-center gap-4 p-4 rounded-3xl border-2 transition-all cursor-pointer ${paymentMethod === "QRPayment" ? "border-primary bg-primary/5" : "border-outline-variant/30 bg-white hover:border-primary/30"}`}
            >
              <input
                type="radio"
                name="paymentMethod"
              ...
              />
              <div
                className={`p-3 rounded-2xl ${paymentMethod === "QRPayment" ? "bg-primary text-white" : "bg-surface-container text-on-surface-variant"}`}
              >
                <QrCode size={24} />
              </div>
              <div className="flex-1">
