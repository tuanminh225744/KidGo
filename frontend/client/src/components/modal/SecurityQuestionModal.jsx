...
import { getKidSecurityQuestion } from '../../../services/kid.service';

export const SecurityQuestionModal = ({ kidId, securityAnswer, setSecurityAnswer, submitSecurityQuestion, errorMsg, loading }) => {
  const [question, setQuestion] = useState("Đang tải câu hỏi...");

  useEffect(() => {
    if (kidId) {
      getKidSecurityQuestion(kidId).then(res => {
        if (res.data?.success) {
           setQuestion(res.data.data.securityQuestion || "Bé chưa được thiết lập câu hỏi bảo mật.");
        }
      }).catch(err => {
         setQuestion("Lỗi tải câu hỏi bảo mật.");
      });
    }
  }, [kidId]);
