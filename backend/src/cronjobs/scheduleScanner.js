...
export const startScheduleScanner = () => {
  ...
  const task = cron.schedule(" * * * * *", async () => {
    try {
      ...
      const nowHh = String(nowMoment.getHours()).padStart(2, "0");
      const nowMm = String(nowMoment.getMinutes()).padStart(2, "0");
      const nowTimeStr = `${nowHh}:${nowMm}`;

      const todayDayOfWeek = targetMoment.getDay(); // 0(CN) - 6(T7)
      const todayStart = new Date(
        targetMoment.getFullYear(),
        targetMoment.getMonth(),
        targetMoment.getDate(),
      );
      ...
      if (nowTimeStr <= targetTimeStr) {
        ...
        query.$and = [
          {
            $or: [
              { pickupTime: { $gte: nowTimeStr } },
              { pickupTime: { $lte: targetTimeStr } },
            ],
          },
        ];
      }
      ...
      console.error(
        "[CronJob Error] Cỗ máy quét Lịch có vấn đề kỹ thuật:",
        err,
      );
    }
  });

  console.log(`[Jobs] Schedule jobs launched successfully.`);
  return task;
};
