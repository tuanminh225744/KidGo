...
export const verifyTripOtp = async (tripId, inputOtp) => {
    try {
        ...
        if (!storedHashedOtp) {
            return { success: false, message: 'Mã OTP không tồn tại hoặc đã hết hạn (TTL)' };
        }
        ...
        if (isMatch) {
            ...
            await Trip.findByIdAndUpdate(tripId, { otpVerified: true }, { returnDocument: 'after' });
            
            // Xoá mã khỏi Redis sau khi dùng thành công
            await redisClient.del(`tripOtp:${tripId}`);
            
            return { success: true, message: 'Xác thực OTP thành công, bắt đầu hành trình' };
        } else {
